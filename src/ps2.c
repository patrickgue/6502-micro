...
/* yes, this is a mess. Must be cleaned up at some point and extended to use
 * other layout than en_US */
int
init_ps2_encodings(ps2_encoding** encoding)
{
  (*encoding) = (ps2_encoding*)malloc(0x80 * sizeof(ps2_encoding));
  int index = 0;
...
void
add_ps2_encoding(ps2_encoding** encoding,
                 int index,
                 char ascii,
                 ps2_scancode code,
                 ps2_modifier modifier)
{
  ...
  (*encoding)[index].sequence =
    malloc((*encoding)[index].sequence_size * sizeof(uint8_t));
...
size_t
encode_ps2(ps2_encoding* encoding,
           ...
           uint8_t** target)
{
  ...
  for (int i = 0; i < encoding_table_size; i++) {
    if (encoding[i].ascii == ascii) {
      ...
      *target = (uint8_t*)malloc(size_actual * sizeof(uint8_t));
      if (useCtrl) {
        ...
        (*target)[encoding[i].sequence_size + 1] =
          (ctrl_mod_code.break_code & 0xff00) >> 8;
        (*target)[encoding[i].sequence_size + 2] =
          (ctrl_mod_code.break_code & 0x00ff);
      } else {
...
char
decode_ps2(ps2_encoding* encoding, uint32_t* source, size_t size)
{
  return 0;
}