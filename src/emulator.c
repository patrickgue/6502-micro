...
zuint8
bus_read(void* context, zuint16 address)
{
...
void
bus_write(void* context, zuint16 address, zuint8 value)
{
  emulator_state* state = (emulator_state*)context;
  if (address < 0xf7fe) {
...
void
init_emulator(emulator_state** state, long clockspeed)
{

  *state = (emulator_state*)malloc(sizeof(emulator_state));
...
size_t
exec_cpu_cycle(emulator_state** state)
{
...
void
tapeinterface_init(emulator_state** state, char* table_file)
{
  char *table, *file;
...
uint8_t
tapeinterface_read(emulator_state** state, bool change_state)
{
    ...
    if (HW_S.tape_byte_position == HW_S.tape_size) {
      ...
      if (HW_S.tape_read_wait) {
        ...
        uint8_t current_byte =
          (HW_S.tape_input_buffer[HW_S.tape_byte_position] >>
...
void
vt100_add_bit(emulator_state** state, uint8_t data)
{
  HW_S.video_buffer[HW_S.video_buffer_size] =
    HW_S.video_buffer[HW_S.video_buffer_size] << 1;
...
void
ps2_send_bit(emulator_state** state)
{
  if (HW_S.ps2_skip_for_next == true) {
    ...
    if (HW_S.ps2_buffer_position != 0) {
      if (HW_S.ps2_buffer_bit_position < 8) {
        HW_S.ps2_current_buffer_bit =
          (HW_S.ps2_buffer[0] & (0b10000000 >> HW_S.ps2_buffer_bit_position)) >
              0
            ? true
            : false;
        ...
        memcpy(
          HW_S.ps2_buffer, HW_S.ps2_buffer + 1, --HW_S.ps2_buffer_position);
...
ps2_add_char_to_buffer(emulator_state** state, int chr, char* ncurses_char_sequence)
{
  ...
  int new_sequence_length = encode_ps2(HW_S.ps2_encoding_table,
                                       ...
                                       ncurses_char_sequence,
                                       &new_sequence);

  HW_S.ps2_buffer =
    realloc(HW_S.ps2_buffer, HW_S.ps2_buffer_position + new_sequence_length);
  memcpy(HW_S.ps2_buffer + HW_S.ps2_buffer_position,
         new_sequence,
         new_sequence_length);
